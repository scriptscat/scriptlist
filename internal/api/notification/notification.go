...
type ListRequest struct {
	...
	ReadStatus            int32 `form:"read_status" binding:"omitempty,oneof=1 2"` // 1:未读 2:已读 不传则全部
	Type                  int32 `form:"type" binding:"omitempty"`                  // 通知类型筛选
}
...
type GetUnreadCountResponse struct {
	...
	Items []*UnreadCountItem `json:"items"` // 分类统计
}
...
type BatchMarkReadRequest struct {
	...
	Type     int32   `json:"type" binding:"omitempty"` // 按类型标记，配合IDs为空使用
}

type BatchMarkReadResponse struct {
	Count int64 `json:"count"` // 标记数量
}
