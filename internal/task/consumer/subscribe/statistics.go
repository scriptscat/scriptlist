...
func (s *Statistics) collect(ctx context.Context, msg *producer.StatisticsCollectMsg) error {
	...
	logger.Ctx(ctx).Info("收集统计日志", zap.Any("msg", msg), zap.String("n", n), zap.Int("len", len(collects)))
	if err := statistics_repo.StatisticsCollect().Create(ctx, collects); err != nil {
		...
		logger.Ctx(ctx).Info("插入成功-collect", zap.Any("msg", msg), zap.String("n", n), zap.Int("len", len(collects)))
	}
	if err := statistics_repo.StatisticsVisitor().Create(ctx, visitors); err != nil {
		...
		logger.Ctx(ctx).Info("插入成功-visitor", zap.Any("msg", msg), zap.String("n", n), zap.Int("len", len(collects)))
	}
	return nil
}
