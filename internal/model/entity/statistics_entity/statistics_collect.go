...
type StatisticsCollect struct {
	...
	OperationHost string `gorm:"column:operation_host;type:varchar(255);not null;comment:操作域名"`
	OperationPage string `gorm:"column:operation_page;type:varchar(255);not null;comment:操作页面"`
...
type StatisticsVisitor struct {
	...
	InstallPage string `gorm:"column:install_page;type:varchar(255);not null;comment:安装页面"`
	InstallHost string `gorm:"column:install_host;type:varchar(255);not null;comment:安装域名"`
}
