...
func ScanKeys(ctx context.Context, match string, keyType string, callback func(ctx context.Context, key string) error) error {
	var list []string
	var cursor uint64
	var err error
	for {
		list, cursor, err = redis.Ctx(ctx).ScanType(cursor, match, 100, keyType).Result()
		if err != nil {
